((logWriter) => {
    ...
    logWriter.init = (app) => {
        ...
        morgan.token('date', (req, res, tz) => {
            return moment().tz('Asia/Kathamandu').format();
        });
        morgan.format('myformat', '[:data[Asia/Kathamandu]]":method:url":status:res[content-lenght]-:response-time ms');
    ...
    logWriter.writeError = (err) => {
        ...
        const logger = createLogger({
            ...
            format: combine(
                label({ label: 'Message' }),
                appendTimestamp({ tz: 'Asia/kathamandu' }),
                myFormat
            ),
            transports: [
                new transports.File({
